...
void set_common_arithmetic_flags(State &state, OperandSize op_size,
  ...
  state.reg_bank.set_flag(state.args.snapshot.flag_transitions, PF,
                          !__builtin_parity(res & 255));

  // Set sign flag (SF)
  // This bit corresponds to the msb of the result
  state.reg_bank.set_flag(state.args.snapshot.flag_transitions, SF,
                          read_msb(op_size, res));

  // Set zero flag (ZF)
  // Set if the result is 0
  state.reg_bank.set_flag(state.args.snapshot.flag_transitions, ZF, res == 0);
}
